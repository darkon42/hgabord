...
int FFT(GABSIGNAL sigin, GABSIGNAL *psigout)
{
  double *xr, *xi;
  int logm,  HalfSize;
  ...
  if(sigin == NULL) perror("FFT: sigin is NULL \n");
  if(sigin->size %2 != 0) 
    perror("FFT: size of complex gabsignal should be even \n");
...
/*********************************************************
* IFFT(input gabsignal, &output gabsignal)                   
**********************************************************/

int IFFT(GABSIGNAL sigin, GABSIGNAL *psigout)
{
  double *xr, *xi;
...
// free tab[MAXLOGM] (CCJ)
// for (i=0; i<MAXLOGM; i++) free(tab[i]); //(static pointers.. cannot free)
