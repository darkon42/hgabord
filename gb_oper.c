 ...
 *				in frequency
 * Bugs:
 *	1. requires the gabsignal size is a power of 2
 *	2. it does not check the validity of 'MacOctave', 'ShiftOctave',
 *	   'SubsampleOctaveTime', 'SubsampleOctaveFreq'.
 */
WORD GaborGetMaxFrmTrans(GABSIGNAL *trans,FILTER *filter, int MinOctave, int MaxOctave, int num_octave,int SubsampleOctaveTime,
...
if ( (MinOctave == 1) && (MaxOctave == LnSigSize - 1) )
	{
    	...
    	complex_array_max(pointer,SigSize,(SigSize>>1)+1, &modula,&value_r,&value_i,&index);
 ...
 *
 * Inputs:
 *	trans		transformation for the gabor library (GABSIGNAL *)
 *	filter		the basic gabor functions	(FILTER *)
 *	word		word selected from trans (WORD)
 *
 * Output:
 *	trans[0]	residue of the project persuit (GABSIGNAL)
 *
 */
 ...
 *	N		the gabsignal size (int)
 *	L		the maximum octave (int)
 *
 * Output:
 *	sWvForm		the gabsignal of the corresponding wave form (GABSIGNAL)
 */
GABSIGNAL GaborGetWaveForm(word,pfFilter,N,L)
WORD word;
FILTER *pfFilter;
int N;
int L;
{
    ...
    GABSIGNAL gabsignal, new_gabsignal();
 ...
 * trans:	transformation matrix (GABSIGNAL *)
 * L:		log2(N)
 ...
 */
void innSigWvForm(gabsignal,filter,j,k,p,L,vr,vi)
GABSIGNAL gabsignal;
FILTER *filter;
int j;
long k;
