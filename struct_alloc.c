...
BOOK AllocBook()
{
  ...
  if(!(book = (BOOK) (malloc(sizeof(struct book)))))
    perror("Mem. alloc for STRUCTURE BOOK failed\n");
...
{
   if (book == (BOOK)NULL)
	perror("init_book(): null argument!");
...
WORD AllocWord()
{
   ...
   if(!(str = (WORD) (malloc(sizeof(struct word)))))
	perror("Mem. alloc for WORD failed\n");
...
INDEX AllocIndex()
{
   ...
   if(!(indx = (INDEX) (malloc(sizeof(struct index)))))
	perror("Mem. alloc for INDEX failed!");
   init_index(indx);
   return(indx);
}
...
INDEX IndexAlloc()
{
	...
	== (INDEX)NULL)
	perror("IndexAlloc(): mem. alloc. failed!");
