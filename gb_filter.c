...
FILTER *GaborAllocFilter(int MaxOctave)
{
    ...
    if ((pfilter=(GABSIGNAL *)malloc((unsigned)((MaxOctave+1)*
		sizeof(struct gabsignal))))== (GABSIGNAL *)NULL)
...
/*
 * build a gabor filter according to the number of octaves and the gabsignal size
 *
 * filter structre:
 * 	see the manual about the gabor dictionary implementation
 *
 ...
 */
FILTER *GaborBuildFilter(int MaxOctave,int SigSize,double sigma,double **fNorm)
{
	...
	void change_gabsignal();
    GABSIGNAL new_gabsignal(int size);
	GABSIGNAL FreeSignal(GABSIGNAL gabsignal);
	GABSIGNAL Gaussian2Signal(double min_t,double max_t,double size_g,int size_s,double sigma);
...
/*
    filterL = new_gabsignal((int)boundN);
    for (n=0;n<boundN;n++)
	{
