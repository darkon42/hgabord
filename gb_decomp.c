 ...
 *				(GABSIGNAL *)
 * Bugs:
 *	1. This function handle a complex gabsignal;
 ...
 *	3. Require 'gabsignal->size' is a power of 2.
 */
					...
					int ShiftOctave)
{
    ...
    int IFFT(), FFT();  /* fft.c */
    int delete_gabsignal(); /* sig_alloc.c */
    void change_gabsignal(), sig_range_copy(), farray_translate(double*, double*, int, int);
    void GaborSubsample();
...
/*
 * put the original gabsignal into trans[0]
 */
    trans[0] = gabsignal;
/*
 * put the Fourier transform of the original gabsignal into trans[MaxOctave - MinOctave + 2]
 */
 ...
 */
	...
	for (k=0;k<=k_length;k++)
		 ...
		 * for the subsample gabsignal {<f,g_j,k,n>} in n with
		 * rate of 2^(j-l)
		 */
		{
...
*/
    ...
    /*
     * normalize the Fourier transform of the orignal gabsignal
     */
    value1 = trans[MaxOctave - MinOctave + 2]->values;
    value2 = value1 + SigSize;
    for (i=0;i<SigSize;i++)
...
/*
 * change the gabsignal size, but dos not change the
 * allocation size
 */
