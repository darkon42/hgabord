...
*      L       $2^L$ is the gabsignal size (int)
...
*
* Output:
*      trans   updated gabor transform (GABSIGNAL *)
*
* Bugs:
...
*	the coarse grid
*/
void UpdateGabor(GABSIGNAL *trans,WORD word, int MinOctave, int MaxOctave, int L,int l,int h,
				 int lf, int hf,double *pfG,double *pfC,double *pfFilterNorm,
				 FILTER *pfFilter,double *pfB,double *pfCE1,int *pnIep)
	{
	/*
	...
	* functions used
	*/
	double GaborGetCoeff(double*,int,int,double*);
	void genericLoop(GABSIGNAL*,int,int,double[2],double[2],double[2],double[2],long,long,long,long,long,long,
		int,int,long,long,int,int,int,int,int,long,long,int,
		unsigned long,double*,int*,double*,double*,double,double);
	void initValues(double[2],double[2],double[2],double[2],long,long,long,long,long,long,double,double*,double,double,unsigned long);
	int delta0(int,long);
...
*
* inputs:
*	trans:	the gabor transformation (GABSIGNAL *)
...
*/
void genericLoop(GABSIGNAL *trans,
				 int	MinOctave, int MaxOctave,
				 double f1_0[2],
				 double f2_0[2],
				 double fs2_0[2],
				 double f3_0[2],
	...
	*/
	void genericRecursion(double f[],long posIndxR,long posIndxI,long negIndxR,long negIndxI,double fmR,double fmI);
	double gbGaussian();
	long P2(long delta,long p1,long j2,long N);
	long K2(long delta,long k1,long Lj2,long N);
	...
	*******************************/
	for (dk=dk0;dk<=dkMaxP;dk+=stepK)
		...
		*/
		...
		mIndx = 1;
		for (m=1; m<mqMax2; m++) {
			gIndx = GINDX(labs(ntmp),maxlh,minjQ,pnIep[n]);
			if (gIndx>=ntmp1) {
				vg[mIndx++] = 0.0;
				} else {
					vg[mIndx++] = pfG[gIndx];
			...
			*/
			if (mFirst)
				...
				*/
				...
				ntmp1 = pnIep[n+1];
				for (m=mLB;m<=mRB ; m++) {
					gIndx = GINDX(labs(ntmp),maxlh,minjM,pnIep[n]);
					ntmp += N;
					if (gIndx<ntmp1) {
						gauss = pfG[gIndx];
						} else {
